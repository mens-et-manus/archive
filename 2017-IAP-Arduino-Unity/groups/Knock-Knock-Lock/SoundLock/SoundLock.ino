...
int background = 62;//backgroun noise (depends on potentiostat tuning)
...
void loop() {
  ...
  if (buttonState==HIGH){//if button pressed
    ...
    if (startTime==0){//initializes startTime
      startTime = millis(); 
    }
    
    if (!isLocked){
      //if button held for a second, set new code
      if ((millis()-startTime)>2000){//if unlocked and held for a long time
        //beep to tell that recording new lock has started
        canLock = false;
        startTime = 0;
        changeKey();
        //reset_lights();//plays the lights for the resetting step
      }else{//
        canLock = true;;
      }
    }
  ...
  if ((millis()-startTime)>waitTime && startTime!=0 && isLocked){
    //get input from rythmic clapping
    //int testTimes [100];
    //testTimes[0] = 260;
    //testTimes[1] = 340;
    //testTimes[2] = -1;
  ...
  //background = 0.9*background + 0.1*analogRead(anl);//adjust the background back by 10%
}//end loop()
