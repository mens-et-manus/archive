...
void endRecordSignal(){
  ...
  int melody_record_end[] = {NOTE_D4, NOTE_D4, NOTE_D4, NOTE_G4, 0};
  int noteDurations_record_end[] = {8, 8, 8, 4, 2};
  for (int thisNote = 0; thisNote < 5; thisNote++) {
    int noteDuration = 800 / noteDurations_record_end[thisNote];
...
void buzzer_record_second(){
  ...
  for (int thisNote = 0; thisNote < 4; thisNote++) {
    int noteDuration = 800 / noteDurations_record_start[thisNote];
...
void buzzer_record_third(){
  ...
  for (int thisNote = 0; thisNote < 6; thisNote++) {
    int noteDuration = 500 / noteDurations_record_start[thisNote];
    tone(BUZZER_PIN, melody_record_start[thisNote], noteDuration);
    digitalWrite(LED_WHITE_PIN, HIGH);
    int pauseBetweenNotes = noteDuration * 1.30;
    delay(pauseBetweenNotes);
