...
// 100s digit
const int LED_3_Pin1 = 23;
const int LED_3_Pin2 = 24;
const int LED_3_Pin3 = 25;
const int LED_3_Pin4 = 26;
const int LED_3_Pin5 = 27;
const int LED_3_Pin6 = 28;
const int LED_3_Pin7 = 29;

// 10s digit
const int LED_2_Pin1 = 30;
const int LED_2_Pin2 = 31;
const int LED_2_Pin3 = 32;
const int LED_2_Pin4 = 33;
const int LED_2_Pin5 = 34;
const int LED_2_Pin6 = 36;
const int LED_2_Pin7 = 35;

// 1s digit
const int LED_1_Pin1 = 37;
const int LED_1_Pin2 = 38;
const int LED_1_Pin3 = 39;
const int LED_1_Pin4 = 40;
const int LED_1_Pin5 = 41;
const int LED_1_Pin6 = 42;
const int LED_1_Pin7 = 43;
...
const unsigned char cap_base = 53;
const unsigned char cap_11 = 50;//50
const unsigned char cap_12 = 47;//47
const unsigned char cap_13 = 44;//44
const unsigned char cap_21 = 45;//45
const unsigned char cap_22 = 48;//48
const unsigned char cap_23 = 51;//51
const unsigned char cap_31 = 46;//46
const unsigned char cap_32 = 49;//49
const unsigned char cap_33 = 52;//52
...
void loop() {
  ...
  Serial.print("{");
  Serial.print(capacitive_values[0][0]);
  Serial.print(", ");
  Serial.print(capacitive_values[1][0]);
  Serial.print(", ");
  Serial.print(capacitive_values[2][0]);
  Serial.print(", ");
  Serial.print(capacitive_values[0][1]);
  Serial.print(", ");
  Serial.print(capacitive_values[1][1]);
  Serial.print(", ");
  Serial.print(capacitive_values[2][1]);
  Serial.print(", ");
  Serial.print(capacitive_values[0][2]);
  Serial.print(", ");
  Serial.print(capacitive_values[1][2]);
  Serial.print(", ");
  Serial.print(capacitive_values[2][2]);
  Serial.println("}");
  
  touch_update();//updates the boolean values of capacitive_values
  
  touch_check();//checks for touches, updates scores, updates display

  set_states();//updates "state" values

  set_display();//updates display based on "states"

  show_score(max(0,score));
  //show_score(max(score, 0));
  
}

void update_leds(){//verified
  ...
  while(duplicate){
    ...
    temp[0] = random(0,width);
    temp[1] = random(0,height);
  ...
  rainbow[num_colors-1][0] = temp[0];
  rainbow[num_colors-1][1] = temp[1];
}


void led_display(int intensity, int color, int x_coord, int y_coord){//verified
  ...
  switch (color){
    ...
    case(color_orange):
      fill_solid(leds, NUM_LEDS, CRGB(64,255,0));
      controllers[i]->showLeds(gBrightness);
      break;
    
    case(color_yellow):
      fill_solid(leds, NUM_LEDS, CRGB(200,255,0));
      controllers[i]->showLeds(gBrightness);
      break;
    ...
    case(background):
      ...
      controllers[i]->showLeds(gBrightness/2);
      break;
    
    case(color_black):
      fill_solid(leds, NUM_LEDS, CRGB::Black);
      controllers[i]->showLeds(0);
    
  }
}

void initialize_rainbow(){//verified
  ...
  for(int i = 0; i < num_colors; i++){
    ...
    while(duplicate){
      rainbow[i][0] = random(0,width);
      rainbow[i][1] = random(0,height);
      duplicate = false;
      for(int j = 0; j < i; j++){
  ...
  for(int i = 0; i < width; i++){
    for(int j = 0; j < height; j++){
      if(capacitive_values[i][j] == true){
        if(rainbow[0][0] == i && rainbow[0][1] == j && event != 1){
          event = -1;
        }
        else{
          event = 1;
...
void show_score(int Number) {//untested
  // put your main code here, to run repeatedly:
