...
void led_display(int intensity, int color, int x_coord, int y_coord){//verified
  ...
  switch (color){
    ...
    case(color_orange):
      fill_solid(leds, NUM_LEDS, CRGB::Red);
      controllers[i]->showLeds(84);
      break;
    
    case(color_yellow):
      fill_solid(leds, NUM_LEDS, CRGB::Red);
      controllers[i]->showLeds(44);
      break;
  ...
  for(int i = 0; i < width; i++){
    for(int j = 0; j < height; j++){
      if(capacitive_values[i][j] == true){
        if(rainbow[0][0] == i && rainbow[0][0] == j && event != 1){
          event = -1;
        }
        else{
          event = 1;
